...
class DPMFSGD(Optimizer):
    def __init__(self, params, lr=1e-3, momentum=0, weight_decay=1, l2_norm_clip=1, noise_multiplier=1,
                 batch_size=500, iterations_number=100, b_min_sep=10, band_width=1, factorization_type='band', MF_coef=None, use_amplification=False, device='cpu', lr_decay_level=1, lr_decay_type='exponential', noise_regeneration_type=None):
        ...
        self.lr_decay_level = lr_decay_level
        ...
        self.noise_regeneration_type=noise_regeneration_type
        if self.noise_regeneration_type is not None:
            generator_device = self.device if self.noise_regeneration_type != 'cpu' else 'cpu'
            self.generator = torch.Generator(device=generator_device)
        print("noise generation", self.noise_regeneration_type)
        ...
        for group in self.param_groups:
          ...
          group['noise_buffer'] = [torch.zeros([noise_buffer_size] + list(param.data.shape), device='cpu') if param.requires_grad else None for param in group['params']]
          group['generator_states'] = [[] for param in group['params']]
    ...
    def generate_noise(self, noise_buffer, grads, coeff, generator_states=None):
      """ Add noise when C is banded"""
      full_noise_multiplier = self.l2_norm_clip * self.C_sens * self.noise_multiplier / self.batch_size
      
      if len(noise_buffer) == 0 or self.band_width == 1:
        new_noise = full_noise_multiplier * torch.randn_like(grads, device=self.device)
        return new_noise.to(self.device), noise_buffer, generator_states
        
      if self.factorization_type == 'band':
        coeff = coeff.to('cpu')
        new_noise = full_noise_multiplier * torch.randn_like(grads, device='cpu')
        ...
        return corr_new_noise.to(self.device), noise_buffer, generator_states

      """ Add noise when C^{-1} is banded"""
      if self.noise_regeneration_type == 'cpu' or self.noise_regeneration_type is None: 
        coeff = coeff.to('cpu')
        new_noise = full_noise_multiplier * torch.randn_like(grads, device='cpu')  
        noise_buffer = torch.cat([new_noise.unsqueeze(0), noise_buffer[:-1]], dim=0)
        inner_shape = noise_buffer.shape[1:]
        return (coeff @ noise_buffer.view(len(noise_buffer), -1)).view(*inner_shape).to(self.device), noise_buffer, generator_states

      """ Noise Regeneration """
      coeff = coeff.to(self.device)
      correlated_noise = torch.zeros_like(grads, device=self.device)
      current_generator_state = self.generator.get_state().clone()
      if len(generator_states) < self.band_width:
          generator_states += [current_generator_state]
      else:
          generator_states = generator_states[1:] + [current_generator_state]

      for i in range(len(generator_states)):
          self.generator.set_state(generator_states[i])
          correlated_noise += torch.randn(correlated_noise.shape, device=self.device, generator=self.generator) * full_noise_multiplier * coeff.flip(0)[i]
      return correlated_noise, noise_buffer, generator_states
        ...
        for group in self.param_groups:
            for param in group['params']:
                if param.requires_grad and param.grad_sample is not None:
                    ...
                    micro_batch_size = grad_samples.shape[0]
                    
                    if total_norm is None:
                        total_norm = torch.zeros(micro_batch_size, device=self.device)
    ...
    def get_lr(self):
        beta = self.lr_decay_level
        n = self.iterations_number
        k = self._step 
        eta = self.defaults['lr']
        
        if self.lr_decay_type == 'exponential':
            return eta * (beta ** ((k - 1) / (n - 1)))
        if self.lr_decay_type == 'polynomial':
            return eta * (beta + (1 - beta) * ((n / k) ** 2 - 1) / (n ** 2 - 1))
        if self.lr_decay_type == 'linear':
            return eta * (1 - (k - 1) / (n -1) * (1 - beta))
        if self.lr_decay_type == 'cosine':
            return eta * (beta + (1 - beta) / 2 * (1 + np.cos((k - 1) / (n - 1) * np.pi)))
        ...
        for group in self.param_groups:
            ...
            for ind, param in enumerate(group['params']):
                if param.requires_grad:
                    ...
                    noise1, group['noise_buffer'][ind], group['generator_states'][ind] = self.generate_noise(group['noise_buffer'][ind], accum_grads, self.matrix_factorization_coefficients, group['generator_states'][ind])
                    ...
                    # Update parameters
                    param.data = param.data * self.weight_decay - group['grad_momentum'][ind] * self.get_lr()
